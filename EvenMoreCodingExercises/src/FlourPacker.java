...
//        However, if goal = 9, bigCount = 1, and smallCount = 5, the method should return true because of 1 full bigCount bag and
...
public class FlourPacker {
    public static boolean canPack(int bigCount, int smallCount, int goal){
        ...
        int checkSum = big + small;

        if (checkSum < goal){
            return false;
        } else if (checkSum == goal){
            return true;
        } else {
            if (goal % 5 <= small){
                return true;
